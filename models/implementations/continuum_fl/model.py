...
Spatiotemporal Graph Neural Network AutoEncoder for Federated Learning PIDS
"""
...
from .utils import create_norm, Pooling


class STGNN_AutoEncoder(nn.Module):
    """
    Spatiotemporal Graph Neural Network AutoEncoder
    
    Encoder-decoder architecture that processes temporal graph snapshots
    using GAT layers and RNN cells for temporal aggregation.
    """
    
    def __init__(self, n_dim, e_dim, hidden_dim, out_dim, n_layers, n_heads, 
                 device, number_snapshot, activation, feat_drop, negative_slope, 
                 residual, norm, pooling, loss_fn="sce", alpha_l=2, use_all_hidden=True):
        super(STGNN_AutoEncoder, self).__init__()

        # Initialize the encoder and decoder structure
        self.encoder = STGNN(
            n_dim, e_dim, out_dim, out_dim, n_layers, n_heads, n_heads, 
            number_snapshot, activation, feat_drop, negative_slope, 
            residual, norm, True, use_all_hidden, device
        )
        self.decoder = STGNN(
            out_dim, e_dim, out_dim, n_dim, 1, n_heads, 1, 
            number_snapshot, activation, feat_drop, negative_slope, 
            residual, norm, False, False, device
        )
    ...
    def forward(self, g):
        ...
        Args:
            g: List of DGL graphs (temporal snapshots)
...
class STGNN(nn.Module):
    ...
    Combines GAT for spatial processing and RNN for temporal aggregation.
    """
    
    def __init__(self, input_dim, e_dim, hidden_dim, out_dim, n_layers, n_heads, 
                 n_heads_out, n_snapshot, activation, feat_drop, negative_slope, 
                 residual, norm, encoding, use_all_hidden, device):
    ...
    def forward(self, G, node_features):
        """
        Forward pass through STGNN
        
        Args:
            G: List of DGL graphs (temporal snapshots)
            node_features: List of node feature tensors
            
        Returns:
            result: List of temporal embeddings
        """
        embeddings = []
        for i in range(len(G)):
            g = G[i]
    ...
    def embed(self, G, node_features):
        ...
        Returns:
            embeddings: Final temporal embeddings
        """
        embeddings = []
        for i in range(len(G)):
            g = G[i].clone()
